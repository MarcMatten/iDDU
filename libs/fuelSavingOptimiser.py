...
def optimise(dirPath, TelemPath):
    try:
        ...
        for i in range(0, len(NBrake)):
            plt.annotate(text='Zone {}'.format(i+1), xy=(d['x'][NBrake][i], d['y'][NBrake][i]), xycoords='data', xytext=(-10, -10), textcoords='offset points', horizontalalignment='right', verticalalignment='top')
        plt.grid()
        plt.axis('equal')
